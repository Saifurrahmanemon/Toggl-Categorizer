...
export function TimeEntryList({ entries }: { entries: CategorizedTimeEntry[] }) {
  ...
  if (entries.length === 0) {
    return <p className="text-gray-500 text-center py-8">No time entries found.</p>
  }
    ...
    <div className="space-y-4">
      {entries.map((entry) => (
        <div key={entry.id} className="border rounded-lg p-4 hover:bg-gray-50 transition-colors">
          <div className="flex justify-between items-start">
            <div className="space-y-1">
              ...
              <p className="text-sm text-gray-500">
                {formatDistanceToNow(new Date(entry.start), { addSuffix: true })} • {Math.round(entry.duration / 60)}{" "}
                minutes
              </p>
            </div>
          ...
          {entry.aiConfidence && entry.aiConfidence < 0.7 && !editingId && (
            <div className="mt-2">
